...
func (j *StatsNotifyJob) UserLoginNotify(username string, ip string, time string, status LoginStatus) {
	...
	if status == LoginSuccess {
		msg = fmt.Sprintf("ورود موفق✅\r\nHostname:%s\r\n", name)
	} else if status == LoginFail {
		msg = fmt.Sprintf("ورود ناموفق❌\r\nHostname:%s\r\n", name)
	}
...
var numericKeyboard = tgbotapi.NewInlineKeyboardMarkup(
    tgbotapi.NewInlineKeyboardRow(
        tgbotapi.NewInlineKeyboardButtonData("📌اطلاعات سرویس📈", "get_usage"),
    ),
)
...
func (j *StatsNotifyJob) OnReceive() *StatsNotifyJob {
    ...
    for update := range updates {
        if update.Message == nil { 
			...
			if update.CallbackQuery != nil {
				...
				switch update.CallbackQuery.Data {
					case "get_usage":
						msg.Text = "📌برای دریافت اطلاعات سرویس : \n <code>/usage ایدی اکانت</code> \n ⁉️مثال :  \n <code>/usage fc3239ed-8f3b-4151-ff51-b183d5182142</code></code>"
						msg.ParseMode = "HTML"
					}
        ...
        case "start":
            msg.Text = "❤️‍🔥سلام به ربات آزاد آکوما خوش اومدی !"
			msg.ReplyMarkup = numericKeyboard
